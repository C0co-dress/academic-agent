...
| 步骤 | Skill | 模式 | 说明 |
|------|-------|------|------|
| 出版级图表 | `nature-figure` | Python/R | 先确定声明→再出图 |
| 论文配套PPT | `nature-paper2ppt` | 默认 | 中文 PPT 用于组会/答辩 |
| 数据可视化 | `scientific-visualization` | 多面板 | 期刊格式化、色盲友好 |

**推荐路径**: `nature-figure` (核心图表) + `nature-paper2ppt` (PPT 版本用于展示)
...
| 步骤 | Skill | 模式 | 说明 |
|------|-------|------|------|
| Nature 风格润色 | `nature-polishing` | 默认（中→英或英→英） | 句子级精修、Nature 措辞 |
| 数据声明 | `nature-data` | 默认 | Nature-ready Data Availability 声明 |
