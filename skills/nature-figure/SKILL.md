...
### Chinese font configuration (Python)

When the figure contains Chinese labels (Simplified or Traditional), replace the font family
settings above with a CJK-aware configuration. Matplotlib does not auto-detect CJK fonts on
most systems; you must set them explicitly.
...
# --- Step 1: find an available CJK font on the system ---
_cjk_candidates = [
    "Source Han Sans SC", "Source Han Sans TC", "Noto Sans CJK SC",
    "Noto Sans SC", "Noto Sans CJK TC", "WenQuanYi Micro Hei",
    "WenQuanYi Zen Hei", "Microsoft YaHei", "PingFang SC",
    "PingFang TC", "Heiti SC", "Heiti TC", "SimHei",
]
_available = {f.name for f in fm.fontManager.ttflist}
_cjk_font = next((f for f in _cjk_candidates if f in _available), None)

if _cjk_font is None:
    # fallback: keep Arial but warn that CJK glyphs will render as tofu
    _cjk_font = "Arial"

mpl.rcParams.update({
    "font.family": "sans-serif",
    "font.sans-serif": [_cjk_font, "Arial", "Helvetica", "DejaVu Sans", "sans-serif"],
    "svg.fonttype": "none",
    "pdf.fonttype": 42,
    "font.size": 7,
    "axes.spines.right": False,
    ...
    "axes.linewidth": 0.8,
    "legend.frameon": False,
    # suppress "Glyph missing from current font" warnings
    "axes.unicode_minus": False,
})
```

**Fallback rule for missing CJK fonts**: If no CJK font is found, stop before rendering and
report which fonts were searched. You may suggest installation commands
(e.g. `pip install matplotlib --upgrade` plus OS-level font install) but must not generate
Chinese-label figures with tofus. If the user only needs English labels, skip CJK font config.

**R CJK fonts** (via `showtext` or `ragg` with `sysfonts`):

```r
library(showtext)
showtext_auto()
font_add("noto", "NotoSansSC-Regular.otf")  # adjust path to local .otf/.ttf
# Then use family = "noto" in ggplot2 themes.
```

### Japanese / Korean (CJK extension)
For JP labels prefer `Source Han Sans JP` / `Noto Sans CJK JP`; for KO prefer
`Source Han Sans KR` / `Noto Sans CJK KR`. The same detection-fallback logic applies.
