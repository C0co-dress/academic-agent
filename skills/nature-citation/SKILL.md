...
### MCP Tool Integration (nature-academic-search)

For broader pre-filtering before narrowing to CNS journals, or when you need multi-source search
beyond Crossref (e.g. PubMed MeSH, arXiv, Semantic Scholar), delegate the search layer to
`nature-academic-search`'s MCP tools first, then feed the candidate DOIs back into this skill
for CNS-journal filtering and reference export.

```
nature-academic-search (broad multi-source search)
    -> candidate DOI list
    -> nature-citation (CNS filter + segment mapping + export)
```

This is especially useful when:
- The topic spans biomedical + computational literature (PubMed + arXiv)
- You need MeSH-based search strategy before CNS filtering
- You need to convert between .nbib / .ris / .bib formats from multiple sources
- Citation verification is needed before CNS-journal scoping
