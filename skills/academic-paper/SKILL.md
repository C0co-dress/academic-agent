...
| Abstract | Bilingual (zh-TW/zh-CN + EN) | Single language |
...
| 7 | `abstract_bilingual_agent` | Bilingual abstract (zh-TW/zh-CN + EN), 5-7 keywords each | Phase 5b |
...
6. **Independent writing** — zh-TW/zh-CN and EN abstracts are independently composed, NOT mechanical translations
...
9. **Word count** — EN: 150-300 words; zh-TW/zh-CN: 300-500 characters
...
Follows the user's language. Academic terminology is kept in English. Bilingual abstracts (zh-TW/zh-CN + EN) are always provided regardless of the main text language. The user can specify zh-TW or zh-CN at the configuration interview; if not specified, follow the user's writing variant.
...
academic-paper + nature-writing           -> Nature-style scaffolds + academic-paper full pipeline
academic-paper + nature-polishing         -> Final prose polish with Nature-leaning English
academic-paper + nature-figure            -> Publication-quality figures embedded in paper
academic-paper + nature-citation          -> CNS/Nature-series citation support per paragraph
academic-paper + academic-paper-reviewer  -> Peer review -> revision loop
academic-paper + nature-response          -> Draft reviewer response letters for revision
```

### Nature-Style Output Path

For users targeting Nature-family or other high-impact journals, the recommended workflow is:

```
[Your Claims + Evidence]
    |
    v
nature-writing (draft scaffold) -> academic-paper full (complete pipeline)
    |                                     |
    v                                     v
nature-figure (figures)          nature-polishing (final prose)
    |                                     |
    +-----> Final Manuscript <-----------+
```

- **nature-writing**: Best for drafting individual sections from author-provided claims and evidence. Use when you have clear results but need help structuring the argument.
- **nature-polishing**: Best for final-pass sentence-level refinement. Run after the academic-paper draft is complete.
- **nature-figure**: Best for individual publication-quality figures. Run in parallel with drafting.
- **nature-citation**: Best for adding CNS/Nature-series citations to already-written paragraphs. Run after the draft stabilizes.

The `academic-pipeline` orchestrator automatically dispatches the right skill at each stage. See `academic-pipeline/references/skills_chain_guide.md` for the complete end-to-end skills chain from research to publication.
