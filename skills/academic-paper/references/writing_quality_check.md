...
## F. Chinese Academic Writing Anti-Patterns (中文学术写作反模式)

This section covers AI-typical patterns specific to Chinese academic writing.
Target: both Traditional (zh-TW) and Simplified (zh-CN) Chinese.

### Flagged AI-Typical Terms (高频AI用语)

| Term | Why it's flagged | Better alternatives |
|------|-----------------|---------------------|
| 值得注意的是 | Filler phrase, adds no information | Delete. Start with the claim directly |
| 具有重要意义 | Vague inflation of importance | State the specific significance |
| 不可或缺 | Overused emphasis | 必要, 关键, or state concretely |
| 至关重要 | Same as above | 关键, 决定性, or be specific |
| 众所周知 | Assumes consensus without evidence | Cite who knows this, or delete |
| 随着...的发展 | Cliché opening pattern | Start with the specific change/mechanism |
| 在...的背景下 | Same as above | State the context directly |
| 越来越 | Vague intensifier | Quantify the trend or be specific |
| 在一定程度上 | Unnecessary hedging unless justified | Delete if the claim is strong; keep only if partial |
| 不可忽视 | Overwrought emphasis | 重要, or state the consequence of ignoring |
| 不容小觑 | Same as above | 值得关注, or be concrete |
| 显而易见 | Claims obviousness without proof | Let the evidence speak for itself |
| 毋庸置疑 | Overclaim | Hedge appropriately: 证据表明, 数据支持 |
| 充分体现 | Vague praise | State what exactly is demonstrated |

### Template Sentence Patterns (模板化句式)

| Pattern | Why it's flagged | Fix |
|---------|-----------------|-----|
| 首先...其次...再次...最后... | Mechanical enumeration; overused by Chinese AI | Vary structure; use 2 good points over 4 forced ones |
| 不仅...而且...同时...还... | Stacked conjunctions, verbose | Split into separate sentences |
| 一方面...另一方面... | Binary contrast overuse | Move beyond dichotomy; use nuanced framing |
| 综上所述 | Lazy conclusion transition | Jump directly to the synthesis point |
| 总而言之 / 总之 | Same as above | State the conclusion without announcing it |
| 基于以上分析 | Meta-commentary filler | Just present the analysis result |
| 本文将从...方面进行探讨 | Throat-clearing ("This paper will explore...") | Start with the finding or argument |

### Structural Patterns (结构反模式)

- **三段式强迫**: Every paragraph uses the same 论点→论据→小结 structure. Vary paragraph rhythm.
- **均匀段落长度**: All paragraphs are ~200-300 characters. Use short (2-3 sentence) paragraphs for emphasis.
- **标题堆砌**: "一、(一) 1. (1)" nested heading chains. Prefer 2 levels max.
- **虚词滥用**: "的"/"了"/"着"/"过" overuse makes sentences flabby. Remove unnecessary 虚词.
- **引号滥用**: Overuse of scare quotes for non-technical terms. Reserve 「」/"" for quotes and special terms.
- **冒号列表**: 2+ consecutive paragraphs each opening with a colon-list structure.

### Punctuation (標點符號)

- **破折号 (——)**: Limit ≤ 2 per paper. AI Chinese text overuses long dashes.
- **省略号 (......)**: Avoid in academic writing. Use specific language instead.
- **分号 (；)**: Limit ≤ 3 per 1000 characters. Prefer periods for sentence separation.

### How to Apply

During the self-review step of Chinese-language drafting, apply these rules alongside sections A-E. The same scoring convention applies (0 violations = clean, 1-3 = minor, 4+ = pattern issue). Fix silently. Do not report scores to the user.
