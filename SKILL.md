...
description: >-
  ...
  plus 2 prompt libraries for Chinese/English polishing, de-AI-ification,
...
metadata:
  ...
  prompt_libraries:
    - references/polishing_prompts_library.md
    - references/figure_generation_prompts.md
    - references/skills_chain_guide.md
  related_skills:
    - academic-paper
    - deep-research
...
| **图表生成** | nature-figure, figure_generation_prompts | 数据/描述 → 出版级图表 |
...
以下 skills 未包含在默认安装中，需手动安装：

| 用户意图 | 路由目标 | 安装方式 |
|---------|---------|---------|
| "ML论文"（NeurIPS/ICML/ICLR） | `ml-paper-writing` | `git clone https://github.com/zechenzhangAGI/AI-research-SKILLs.git` |
| "系统论文"（OSDI/SOSP/ASPLOS） | `systems-paper-writing` | 同上 |
| "做Beamer/会议演讲PPT" | `presenting-conference-talks` | 同上 |
| "论文架构图/概念图" | `canvas-design` | `git clone https://github.com/anthropics/skills.git` |
| "协作写文档" | `doc-coauthoring` | 同上 |
| "去AI味"（系统化） | `humanizer` | `git clone https://github.com/blader/humanizer.git` |
...
```
用户: "帮我做论文的图表"
→ 正式数据图: nature-figure / figure_generation_prompts
→ 架构图/概念图: figure_generation_prompts §1
...
本 Agent 所有自定义文件均位于 `academic-agent/` 目录（自包含，克隆即用）：
